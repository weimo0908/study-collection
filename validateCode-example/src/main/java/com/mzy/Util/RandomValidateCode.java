...
public class RandomValidateCode {
	 ...
	 * @param request
	 * @param response
	 * @param key 用于存储在cookie或session中的key
	 */
	public void getRandomcode(HttpServletRequest request, HttpServletResponse response, String key) {

		// 创建图片 BufferedImage类是具有缓冲区的Image类,Image类是用于描述图像信息的类
		BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_BGR);
		//获取绘图对象 ,改对象可以在图像上进行各种绘制操作
		...
		//设置反色
		g.setColor(getReverseColor(getRandomColor()));
		// 绘制干扰线
		...
		request.getSession().setAttribute(key + "_" + sessionid, randomString);
		System.out.println("*************" + randomString +sessionid);
